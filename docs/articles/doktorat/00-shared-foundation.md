...
**Status:** normative reference. Every article in the cycle MUST conform to the notation (§2), draw citations only from the master bibliography (§3), state hypotheses consistently with (§4), reuse the recompute table in (§5) without recomputation, and close with the Claims/Non-Claims box (§6). Frozen against verification commit `a1063f9`; model-correction changeset 2026-07-02 (see CHANGELOG); legal validity date **2026-06-28**. Sources of truth: `docs/VERIFICATION_REPORT.md` (citation table §2, parameter table §3, recompute §7), `docs/MODEL_PARAMETERS.md`, `RESEARCH.md`, `docs/EMPIRICAL_VALIDATION_PLAN.md`.
...
5. **Renegotiation** (`C_reneg`): expected renegotiation cost *associated with* rigidity (2SLS-IV import, not a clean causal transfer). **One formula for both paths**: `P(ρ) = 0.22 + min(0.077 × ρ × m_ctx, 0.105)`, the flexible path evaluated at ρ_F = min(ρ, 0.15). Mapping assumption: the full 0→1 swing of the rigidity index ≈ one standard deviation, anchored at the 7.7pp lower bound and hard-capped at the 10.5pp upper bound of the Beuve band (`RENEGOTIATION_PREMIUM_MAX` = 0.105).
6. **TCO** (`C_TCO`): foregone total-cost-of-ownership savings, **discounted** (d = 0.05) and **capped**. `C_TCO = V × m_ctx × min(γ × A(T,d) × ρ, κ_TCO)`, γ = 0.10/yr, κ_TCO = 0.30; the context multiplier sits *outside* the cap, so the effective ceiling is 0.30 × m_ctx (max 0.345).
7. **Bypass** (`C_bypass`): expected audit/penalty cost of informal circumvention. **One formula for both paths**: `p(ρ) = clamp(σ(ρ) × m_tech × m_ctx, 0, 0.95)`, with σ a sigmoid (steepness 6, threshold 0.9) and both multipliers applied on the sigmoid *output*; the flexible path is evaluated at the tech level's `policyRigidityIndex`. Lands a maximally-rigid manual process near ~86% and falls to ~6% under end-to-end digital tooling.
...
*Use exactly:* discretion raises normalized price by **~6 percent (structural causal estimate; reduced-form fuzzy-RD ~9%; selection-corrected ~8%)** and selects **~10% less-productive** contractors (structural estimate). Never write the price effect as "percentage points". Hungarian public-procurement RDD around a 25M HUF reform threshold + structural selection-correction; the **structural estimates correct for selection into tenders** (read δ = 0.06 as a transfer-laden benchmark, not a measurement; value-manipulation/bunching caveat). **Competitive tendering averts the favoritism premium.** External validity: Hungarian public → generic/PL/private is a transfer, not a measured Polish value.
...
*Use exactly:* adaptation costs **7.5–14% of the winning bid** (version-of-record; author PDF 8–14%; working-paper ~10%). US Caltrans highway-paving; observational + structural with engineer-identity IV and firm FE. **Do NOT reuse as the TCO basis; it double-counts with the renegotiation dimension.**
...
*Use exactly:* average procedure cost **~€28,000 (75% supplier-side)**, **5.4 bidders**, **38 days**, **€5.26 bn** aggregate, **~1.4% of purchasing volume**; regressivity 18–29% / 6–9%. EU above-threshold public, EEA-30, 2009. Caveat: €28k is *total* procedure cost, not the rigidity premium; only ~0.4% of value (~€1.68 bn) is Directive-incremental.

**TCO practitioner ceiling — unattributed grey-literature heuristic.** *(Formerly cited as "Institute for Supply Management (n.d.)"; the 2026-07 citation-verification pass traced the quote to a content farm squatting the ism.ws domain — there is no verifiable ISM or peer-reviewed source.)*
*Use exactly:* an **unattributed practitioner heuristic (grey literature)** holds that TCO-based sourcing "can deliver **up to 30%** … over three years": a **ceiling / best-case**, not a flat ~10%/yr empirical rate; no control/counterfactual; no verifiable institutional provenance. Used only as the Grade-C cumulative cap κ_TCO = 0.30. **Do not attribute the figure to ISM or to CAPS Research.** Premise of the TCO dimension is anchored to Decarolis (2014), not to this heuristic.
...
- **Swiss Casinos ERP (Lean Agile Procurement)**: LAP Alliance (lean-agile-procurement.com), Swiss Casinos case study; World Procurement Awards 2020 winner. Correct figures: **~6 weeks (business case → proof of concept + signed contract) vs ~6 months** for a comparable classic sourcing cycle (~75% faster). Do **not** attribute to EY Switzerland or Skylight Digital, and do not use the earlier "4 weeks" or "120-day vs 28-day" renderings.
- **Air France KLM Martinair cargo**: LAP Alliance / Agile Business Consortium (2021), *Air France Uses Lean Agile Procurement to Outsource a Critical Project* (lap-alliance.org; agilebusiness.org). Correct figures: cargo booking system with a **six-month delivery deadline**; vendor selection closed in **~6 weeks** (two-day "POCAthlon" with 4 pre-selected vendors) vs traditional RFP processes "usually lasting several months". Do **not** attribute to EY Switzerland, and do not use the earlier unsupported "12–18 months" rendering.
...
Real-code recompute via the permanent in-repo script **`scripts/recompute.ts`** (run: **`npm run recompute`**), which imports the *actual* `calculateCosts` + `SCENARIOS` (no re-implementation) and replaces the earlier out-of-repo harness. The numbers reflect the **2026-07-02 model correction**: symmetric renegotiation and bypass formulas applied to both paths, removal of the staff double-count (role-level multipliers in `deriveStaffCost` are now the sole staff channel), the renegotiation premium hard-capped at 10.5pp, and recalibrated context multipliers under the audited invariant that no dimension's total context uplift exceeds ~×1.5 (max observed ×1.483). All 9 reference scenarios computed. All scenarios leave `spendType`/`processPhase` unset → Direct/Indirect × Upstream/Downstream multipliers = 1.0 (dormant for shipped cases). Source: `docs/VERIFICATION_REPORT.md` §7 + `scripts/recompute.ts`.
...
`deltaC` is **strictly positive in every shipped scenario**: flexible is net-cheaper everywhere, *including* the maximal-stakes `pipe_vs_field` case (pzp_eu, `CORRUPTION_RISK_CONTEXT = 1.0`, ρ = 0.95), which is in fact the **single widest gap** as a share of contract value (+35.8%). The favoritism dimension *does* favor the rigid path in **8/9** scenarios (Δ from −11k to −308k; `mrp` is exactly 0 because ρ = 0.12 ≤ the 0.15 policy floor), so governance value is credited **per-dimension**, but it never wins at net. Among the 9 scenarios the dominant dimension is `tco` in 4 (fleet, logistics, pipe_vs_field, capex) and `opp` in 3 (erp, production, custom), with `catalog` and `mrp` (both admin-dominated, +3.23% and +0.28% of CV) the two sub-threshold micro-cases; i.e., the gap is driven by `tcoCost` (4) or `opportunityCost` (3) in every value-significant case.
...
> 6. All imported effects carry their **identification and external-validity caveats** (Szucs: Hungarian RDD, structural estimates correct for selection into tenders; Beuve: French car-parks, 2SLS/IV; Guasch: LAC ex-telecom; Bajari-Houghton-Tadelis: US Caltrans; Decarolis/Coviello-Mariniello: Italian works). Transfers to the Polish context are benchmarks, not measurements.
...
| **Article 2** | *Ile kosztuje sztywność? Symetryczny, wielowymiarowy model kosztu proceduralnego w zamówieniach* (PL) | **Anchors *ekonomia i finanse*** | Szucs (2024); Beuve et al. (2021/2023); Guasch (2004); Bajari-Houghton-Tadelis (2014); Decarolis (2014); Coviello-Mariniello (2014); EC PwC (2011); TCO practitioner ceiling (unattributed). **Owns** the closed-form specification of `ΔC_total = Σ ΔCᵢ`, the A/B/C parameter-provenance taxonomy, the deterministic recompute, the sensitivity sweeps (TCO yr1 6/8/12%; `CORRUPTION_RISK_CONTEXT` gradient; `PROCESS_RIGIDITY` pzp_eu=0.95; `RENEGOTIATION_PREMIUM_MAX` ∈ [0.077, 0.105] and the SD-mapping), and binds paths to PZP arts. 132/140/169/153/214/275. | Article 1's notation + decomposition; **§5 recompute table verbatim**, §6 box |
