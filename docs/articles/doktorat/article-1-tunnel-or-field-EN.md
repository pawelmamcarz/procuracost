...
**Doctoral-cycle article 1 | conceptual paper | ProcuraCost 2.3.0 | draft**
...
- system support;
- contract design;
- organisational implementation readiness.

These choices interact, but they are not interchangeable. An open procedure can
use parallel internal preparation. A private purchase can follow a heavily
gated sequence. A detailed contract can follow a short sourcing process, while
a modular contract can emerge from a formal public procedure.

Earlier ProcuraCost versions combined some of these constructs in aggregate
path profiles and a prescriptive optimiser. Native model 2.3 represents the two
workflow designs independently and keeps readiness outside the cost model.
...
Let the boundary contain applicable authorisations, competition duties, ethical
requirements and documentation obligations. Let a workflow specify how
activities proceed inside that boundary.

A **tunnel** is a workflow in which activities and approvals are substantially
pre-ordered. It can support predictability, repeatability and a clear decision
trail. It can also create queues, repeated hand-offs and late discovery of
requirement problems.

A **field** is a workflow in which an authorised team may parallelise, revisit
or select activities while the same boundary remains in force. It can support
earlier learning and different sequencing. It can also require additional
...
2.2.2, then allocate those totals and role hours across named steps before
applying retained support profiles. The official sources below support only the
qualitative mechanism. They do not supply step days, role hours or cost effects.

### 4.1 ERP transformation discovery
...
catalogue are established. An MRP release can follow an identical map when it
uses an existing agreement and approved material data.

The stable-service starting scenario is a workflow-topology control only. It
...
starting alternative as restricted. The allocation can be reversed or removed;
...
be fully specified ex ante (Bajari, Houghton and Tadelis 2014). Beuve, Moszoro
and Spiller (2023) study contractual rigidity and formal-amendment frequency in
French car-park contracts. Their empirical object is not the number of sourcing
approvals or the topology of a procurement workflow.

Native model 2.3 therefore starts contract-amendment and TCO differentials at
...
- **P1a:** Greater ex ante workflow prescriptiveness is associated with more
  elapsed days from approved need to contract signature, conditional on
  purchase complexity, legal boundary, procedure family and applicable
  mandatory-wait days. **P1b:** under the same controls it is associated with
  more buyer, legal and manager coordination hours per procedure.
- **P2:** Broader ex ante market access is associated with more valid bids and
  a lower award price relative to a preregistered independent benchmark,
  independently of workflow topology.
- **P3:** Greater ex ante rigidity of contract-change clauses is associated
...
- **P4:** For requirements classified as incomplete before process design,
...
- **P5:** A higher logged completion rate for required system controls is
  associated with fewer off-system approvals or orders and fewer missing audit
  events. System ownership and readiness responses are not operational
  exposure measures.

The propositions permit either alternative to have the lower cost and permit a
null result. P1a, P1b and P2 to P5 operationalise the canonical H1a, H1b and H2
to H5 in `00-shared-foundation.md`.
...
Contract-amendment and TCO differentials are zero. Bypass is non-monetised.
...
8](https://www.youtube.com/watch?v=5KYUdTLlvvg) raises practical questions about
internal ownership, process friction, requirement discipline, operational
purchasing, policy simplification, TCO and bounded use of AI. The recording is
in Polish and its transcript provenance is automatic YouTube captions.

The material informs interview prompts and hypotheses only. It cannot set a
workflow duration, cost, competition stress, readiness responses or threshold.
Bielik may structure market data for human review. The deterministic model
performs the calculation.

## References

Bajari, P., Houghton, S., and Tadelis, S. (2014). Bidding for Incomplete
...
Beuve, J., Moszoro, M. W., and Spiller, P. T. (2023). Doing It by the Book:
...
Coviello, D., and Mariniello, M. (2014). Publicity Requirements in Public
