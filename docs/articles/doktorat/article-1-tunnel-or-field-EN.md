...
> The probability of (ii) is strictly increasing in the gap between τ and the accommodation capacity of Aᵢ. Critically, **no third option of *compliant adaptation* exists**, because under the conflation compliance is defined as identity with Aᵢ.
...
The third mechanism is ideological. Kelman (1990), examining U.S. federal procurement, diagnosed a pervasive *fear of discretion*: a conviction that the way to secure integrity and performance is to *remove* the official's judgment, replacing it with rule-bound process. The fear is not baseless — discretion can be abused (Section 6 takes this seriously) — but the response it motivates is to wall in the official as tightly as possible, which is to build the tunnel.
