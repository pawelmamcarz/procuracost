...
5. Design diagnostics only when enough of the running variable is present: McCrary and Cattaneo-Jansson-Ma density tests at the 130,000 PLN threshold and the EU thresholds; covariate balance; bandwidth sensitivity. Report bunching as a result (an observable avoidance signature), and treat it as a threat to the sharp RDD that must be disclosed.
