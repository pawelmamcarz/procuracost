...
**Bypass-rigidity input by path (corrected 2026-07-05).** The rigid path's bypass sigmoid is evaluated at the process rigidity ρ; the flexible path's at the **tech level's** `policyRigidityIndex` (0.05–0.35), reflecting that manual tooling makes even a policy process easier to bypass. This tech-level index was previously used **uncapped**, so for a low-rigidity operational process (`catalog_order` ρ = 0.20, `mrp_order` ρ = 0.12) on **manual** (0.35) or **sourcing_tool** (0.22) tooling the flexible "field" path was scored MORE bypass-prone than its own rigid "tunnel" path — inverting the Tunnel/Field thesis. **Fix:** the flexible-path input is now `min(policyRigidityIndex, flexibleRigidity)` where `flexibleRigidity = min(ρ, ρ_policy_only)`, so the field path can never read as more rigid than the tunnel. Observed effect on the shipped reference scenarios (which use `end_to_end` tooling for the operational cases, so those are unaffected): the two `private_formal` scenarios on non-`end_to_end` tooling shift — **erp** (sourcing_tool) flexible bypass falls, deltaC +802k → **+803k** (+26.73% → +26.77% CV); **production** (manual) flexible bypass falls, deltaC +2.26M → **+2.30M** (+18.83% → +19.20% CV). The catalog/mrp inversion is only realized under manual/sourcing_tool tooling (covered by the new regression tests). The symmetry test is unchanged (still 0/9 deltaC < 0).
...
- `FLEXIBLE_RENEGOTIATION_PROBABILITY_FACTOR` (0.70) and `FLEXIBLE_BYPASS_PROBABILITY_SCALE` (0.10) — removed 2026-07-02. Both were flexible-only ad-hoc discounts that double-encoded the rigidity→risk mechanism and violated the symmetric-model invariant; superseded by shared per-path formulas (renegotiation: `P(ρ)` at each path's own rigidity; bypass: one sigmoid, flexible path evaluated at `min(policyRigidityIndex, flexibleRigidity)` — see the "Bypass-rigidity input by path" note above; the earlier uncapped `policyRigidityIndex` reading inverted the Tunnel/Field thesis for low-rigidity operational processes and was corrected 2026-07-05).
