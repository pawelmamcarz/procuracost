...
export const comparisonT = {
  pl: {
    costLabels: {
      ...
      productivityCost: "Korekta produktywności (wyłączona)",
      renegotiationCost: "Renegocjacje",
      tcoCost: "Modelowana utracona możliwość TCO",
      bypassCost: "Modelowana ekspozycja na obejścia",
    },
    deltaHeadline: "Modelowana różnica kosztów ścieżek",
    higherThan: "wyższy w modelu niż dla podejścia opartego na polityce zakupowej",
    bypassLabel: "Modelowane prawdopodobieństwo obejścia",
    bypassNote:
      "Wynik funkcji sigmoidalnej o niezwalidowanych parametrach; wymaga danych o transakcjach poza systemem",
    ...
    colDiff: "Różnica",
    sourcesTitle: "Źródła i założenia modelu",
    importance: "Ważność",
    ...
    pipeDesc:
      "Sztywna procedura wyznacza jeden tor: a₁ → a₂ → a₃ → ... → aₙ. Model zakłada, że niedopasowanie toru i presja mogą zwiększać obejścia; hipoteza wymaga obserwacji.",
    fieldLabel: "Polityka zakupowa = Pole",
    fieldDesc:
      "Polityka wyznacza pełny zbiór granic prawnych i organizacyjnych. Może dopuszczać kilka autoryzowanych ścieżek, ale obejścia i nadużycia nadal są możliwe i wymagają kontroli.",
    pipeFieldSource: "Inspiracje teoretyczne: Lipsky (1980), Vaughan (1996), Holmström & Milgrom (1991); parametry obejść są założeniami",
    benchmarkTitle: "Twój wynik na tle syntetycznych scenariuszy",
    benchmarkSubtitle: "Modelowana różnica ścieżki sztywnej (% ponad koszt ścieżki elastycznej)",
    benchmarkYours: "Twój scenariusz",
    benchmarkSummary: (pct: number, rank: number, total: number) =>
      `Model daje dla Twoich danych różnicę ${pct}% — wyższą niż w ${rank} z ${total} syntetycznych scenariuszy.`,
  },
  en: {
    costLabels: {
      ...
      productivityCost: "Productivity adjustment (inactive)",
      renegotiationCost: "Renegotiation",
      tcoCost: "Modeled foregone TCO opportunity",
      bypassCost: "Modeled bypass exposure",
    },
    deltaHeadline: "Modeled path cost difference",
    higherThan: "higher in the model than the policy-based approach",
    bypassLabel: "Modeled bypass probability",
    bypassNote:
      "Output of an unvalidated sigmoid function; requires observed off-system transaction data",
    ...
    colDiff: "Difference",
    sourcesTitle: "Sources and model assumptions",
    ...
    pipeDesc:
      "A rigid procedure defines one path: a₁ → a₂ → a₃ → ... → aₙ. The model assumes that path mismatch and pressure may increase bypass; this hypothesis requires observation.",
    fieldLabel: "Procurement policy = Field",
    fieldDesc:
      "Policy defines the complete legal and organizational constraint set. It may permit several authorized paths, but bypass and misconduct remain possible and require controls.",
    pipeFieldSource: "Theoretical motivations: Lipsky (1980), Vaughan (1996), Holmström & Milgrom (1991); bypass parameters are assumptions",
    benchmarkTitle: "Your result vs synthetic scenarios",
    benchmarkSubtitle: "Modeled rigid-path difference (% above flexible-path cost)",
    benchmarkYours: "Your scenario",
    benchmarkSummary: (pct: number, rank: number, total: number) =>
      `The model gives your inputs a ${pct}% difference — higher than ${rank} of ${total} synthetic scenarios.`,
  },
} as const;
...
export const optimizerT = {
  pl: {
    ...
    modelConfidence: "Zgodność wariantów",
    treeVotes: "Głosy wariantów (z 30)",
    typicalTime: "Typowy czas",
    pzpNote: "Nota PZP",
    rankingTitle: "Ranking wszystkich ścieżek (30 wariantów scoringu)",
    importanceTitle: "Lokalna wrażliwość rekomendacji",
    importanceNote:
      "Zmiana poparcia dla rekomendowanej ścieżki po zastąpieniu jednego parametru wartością referencyjną. Skala względna: największy lokalny wpływ = 100%.",
    ...
    modelNote:
      "Metoda: transparentny ensemble 30 deterministycznych wariantów scoringu opartych na jawnych regułach eksperckich. To heurystyka wsparcia decyzji, nie wytrenowany klasyfikator ML ani weryfikacja prawna.",
    importanceUnit: "%",
    importance: "Ważność",
  ...
  en: {
    ...
    modelConfidence: "Variant agreement",
    treeVotes: "Variant votes (of 30)",
    typicalTime: "Typical time",
    pzpNote: "PZP note",
    rankingTitle: "All paths ranked (30 scoring variants)",
    importanceTitle: "Local recommendation sensitivity",
    importanceNote:
      "Change in support for the recommended path after replacing one input with its reference value. Relative scale: largest local effect = 100%.",
    ...
    modelNote:
      "Method: transparent ensemble of 30 deterministic scoring variants built from explicit expert rules. It is a decision-support heuristic, not a trained ML classifier or legal compliance check.",
    importanceUnit: "%",
    importance: "Importance",
