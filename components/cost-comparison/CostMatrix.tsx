...
function matrixColorClass(cost: number, min: number, max: number): string {
  const ratio = max === min ? 0.5 : (cost - min) / (max - min);
  if (ratio < 0.25) return "text-gray-500";
    ...
    <>
              ...
              {TECH_LEVEL_IDS.map((tl) => {
                ...
                return (
                  <tr key={tl} className={isActive ? "ring-1 ring-inset ring-blue-300 bg-blue-50" : "hover:bg-gray-50"}>
                    ...
                    <td
                      className={`px-3 py-2 text-right font-mono font-semibold tabular-nums ${matrixColorClass(rigidCell.totalCost, minCost, maxCost)}`}
                    >
                      {formatPLN(rigidCell.totalCost)}
                    </td>
                    <td className="px-3 py-2 text-right font-mono text-gray-400 tabular-nums">{rigidCell.days}</td>
                    <td
                      className={`px-3 py-2 text-right font-mono font-semibold tabular-nums ${matrixColorClass(flexCell.totalCost, minCost, maxCost)}`}
                    >
                      {formatPLN(flexCell.totalCost)}
                    </td>
                    <td className="px-3 py-2 text-right font-mono text-gray-400 tabular-nums">{flexCell.days}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {(inputs.spendType || inputs.processPhase) && (
        <div className="mt-3 rounded-lg border border-blue-100 bg-blue-50/60 px-3 py-2 text-[11px]">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-blue-700 tabular-nums">
